...
//update cabang
mysqli_query($koneksi, "UPDATE sekolah_uang_masuk SET cabang = '$e_cabang' ".
...
if ($_POST['btnSMP'])
	...
	if ((empty($e_kode)) OR (empty($e_tipe)) OR (empty($e_nama)))
		...
		if ($s == "baru")
			{
			mysqli_query($koneksi, "INSERT INTO sekolah_uang_masuk(kd, sekolah_kd, sekolah_kode, sekolah_nama, ".
							"per_tgl, tipe, kode, ".
							"nama, nilai, ket, postdate) VALUES ".
							"('$x', '$sekkd84_session', '$e_sekkode','$e_seknama', ".
		...
		if ($s == "edit")
			{
			mysqli_query($koneksi, "UPDATE sekolah_uang_masuk SET tipe = '$e_tipe', ".
							"kode = '$e_kode', ".
							"nama = '$e_nama', ".
							"nilai = '$e_nilai', ".
							"ket = '$e_ket', ".
...
if ($_POST['btnHPS'])
	...
	for ($i=1; $i<=$jml;$i++)
		...
		//del
		mysqli_query($koneksi, "DELETE FROM sekolah_uang_masuk ".
						"WHERE kd = '$kd'");
		}
...
if (($s == "baru") OR ($s == "edit"))
	...
	$qx = mysqli_query($koneksi, "SELECT * FROM sekolah_uang_masuk ".
						"WHERE kd = '$kdx'");
	$rowx = mysqli_fetch_assoc($qx);
	...
	if (empty($kunci))
		{
		$sqlcount = "SELECT * FROM sekolah_uang_masuk ".
						"WHERE sekolah_kd = '$sekkd84_session' ".
	...
	<td align="center"><strong><font color="'.$warnatext.'">TIPE SUMBER</font></strong></td>
