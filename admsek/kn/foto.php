...
$judulku = "[KIRIM NASKAH]. Galeri Foto";
$judulx = $judul;
$s = nosql($_REQUEST['s']);
...
if($_POST['btnSMP'])
	...
	if (empty($e_judul))
		...
		{
		//query
		mysqli_query($koneksi, "INSERT INTO cp_foto (kd, user_kd, user_kode, user_nama, user_posisi, ".
									"nama, postdate) ".
									"VALUES ('$kdku', '$kd81_session', '$e_kode', '$e_nama', 'SEKOLAH', ".
									"'$e_judul', '$today');"); 
		...
		$ku_ket = cegah("[MENU : $judul]. KIRIM NASKAH FOTO : $e_judul");			
		...
		//re-direct
		$pesan = "Terima Kasih. Naskah Foto, akan terlebih dahulu dilakukan verifikasi oleh Pihak Majelis.";
		pekem($pesan,$filenya);
		exit();
		}
...
if($_POST['btnHPS'])
	...
	for ($i=1; $i<=$limit;$i++)
		...
		//del
		mysqli_query($koneksi, "DELETE FROM cp_foto ".
...
if (($s == 'baru') OR ($s == 'edit'))
	{
	//query
	$qx = mysqli_query($koneksi, "SELECT * FROM cp_foto ".
									"WHERE user_kd = '$kd81_session' ".
									"AND kd = '$kdku'");
	$rowx = mysqli_fetch_assoc($qx);
	$e_judul = balikin($rowx['nama']);
	...
	$(document).ready(function(){
	       ...
	       $('#image-holder').load("<?php echo $sumber;?>/admsek/kn/i_foto.php?aksi=lihat1&sekkd=<?php echo $kd81_session;?>&kd=<?php echo $kd;?>");
			...
			$('#loading').show();
		     ...
		     $.ajax({
		      ...
		      success:function(data){
				$('#loading').hide();
		       $('#preview').load("<?php echo $sumber;?>/admsek/kn/i_foto.php?aksi=lihat&sekkd=<?php echo $kd81_session;?>&kd=<?php echo $kd;?>");
	  ...
	  <table class="table" border="1">
		...
		do
			...
			if ($i_verifikasi == "false")
				...
				{
				$i_ket = "<font color='green'>NASKAH FOTO DITERIMA. <br>$i_verifikasi_postdate</font>";
				}
