...
//nilai
$filenya = "youtube.php";
$judul = "[KIRIM NASKAH]. Link Youtube";
$judulku = "[KIRIM NASKAH]. Link Youtube";
$judulx = $judul;
$s = nosql($_REQUEST['s']);
...
if($_POST['btnSMP'])
	...
	//nek null
	if ((empty($e_judul)) OR (empty($e_filex)))
		{
		//re-direct
		...
		{
		//query
		mysqli_query($koneksi, "INSERT INTO cp_video (kd, user_kd, user_kode, user_nama, user_posisi, ".
									"judul, filex, postdate) ".
									"VALUES ('$kdku', '$kd81_session', '$e_kode', '$e_nama', 'SEKOLAH', ".
									"'$e_judul', '$e_filex', '$today');"); 
		...
		//re-direct
		$pesan = "Terima Kasih. Naskah Link Youtube, akan terlebih dahulu dilakukan verifikasi oleh Pihak Majelis.";
		pekem($pesan,$filenya);
		exit();
		}
...
if($_POST['btnHPS'])
	...
	for ($i=1; $i<=$limit;$i++)
		...
		//del
		mysqli_query($koneksi, "DELETE FROM cp_video ".
	...
	$ku_ket = cegah("[MENU : $judul]. HAPUS : $e_judul");			
...
if (($s == 'baru') OR ($s == 'edit'))
	{
	//query
	$qx = mysqli_query($koneksi, "SELECT * FROM cp_video ".
									"WHERE user_kd = '$kd81_session' ".
									"AND kd = '$kdku'");
	$rowx = mysqli_fetch_assoc($qx);
	...
	URL Youtube : 
	<br>
	<input name="e_filex" id="e_filex" type="text" value="'.$e_filex.'" size="50" class="btn btn-warning" required>
	</p>
