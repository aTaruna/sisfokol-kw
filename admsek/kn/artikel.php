...
//nilai
$filenya = "artikel.php";
$judul = "[KIRIM NASKAH]. Artikel/Info/Berita";
$judulku = "[KIRIM NASKAH]. Artikel/Info/Berita";
$judulx = $judul;
$s = nosql($_REQUEST['s']);
...
if($_POST['btnSMP'])
	...
	if ((empty($e_kategori)) OR (empty($e_judul)))
		...
		{
		//query
		mysqli_query($koneksi, "INSERT INTO cp_artikel (kd, user_kd, user_kode, user_nama, ".
						"user_posisi, kategori, judul, isi, postdate) ".
						"VALUES ('$kdku', '$kd81_session', '$e_kode', '$e_nama', ".
						"'SEKOLAH', '$e_kategori', '$e_judul', '$e_isi', '$today');"); 
		...
		$ku_ket = cegah("[MENU : $judul]. KIRIM NASKAH : $e_judul");			
		
		
		//insert
		mysqli_query($koneksi, "INSERT INTO user_log_entri(kd, user_kd, sekolah_kode, sekolah_nama, ".
						"user_kd, user_kode, user_nama, ".
						"user_posisi, user_jabatan, ket, verifikasi, postdate) VALUES ".
						"('$x', '$ku_kd', '$ku_kode', '$ku_nama', ".
						"'$ku_kd', '$ku_kode', '$ku_nama', ".
						"'SEKOLAH', 'TATA USAHA', '$ku_ket', 'false', '$today')");
		//kasi log login ///////////////////////////////////////////////////////////////////////////////////
		...
		//re-direct
		$pesan = "Terima Kasih. Naskah Artikel/Berita/Info, akan terlebih dahulu dilakukan verifikasi oleh Pihak Majelis.";
		pekem($pesan,$filenya);
		exit();
		}
...
if($_POST['btnHPS'])
	...
	for ($i=1; $i<=$limit;$i++)
		...
		mysqli_query($koneksi, "DELETE FROM cp_artikel ".
						"WHERE user_kd = '$kd81_session' ".
						"AND verifikasi = 'false' ".
						"AND kd = '$kd'");
		}
...
if (($s == 'baru') OR ($s == 'edit'))
	...
	$(document).ready(function(){
	       ...
	       $('#image-holder').load("<?php echo $sumber;?>/admsek/kn/i_artikel.php?aksi=lihat1&sekkd=<?php echo $kd81_session;?>&kd=<?php echo $kd;?>");
			...
			$('#loading').show();
		     ...
		     $.ajax({
		      ...
		      success:function(data){
				$('#loading').hide();
		       $('#preview').load("<?php echo $sumber;?>/admsek/kn/i_artikel.php?aksi=lihat&sekkd=<?php echo $kd81_session;?>&kd=<?php echo $kd;?>");
	...
	if ($count != 0)
	  ...
	  <table class="table" border="1">
		...
		do
			...
			$i_verifikasi = balikin($data['verifikasi']);
			$i_verifikasi_postdate = balikin($data['verifikasi_postdate']);
			...
			//status
			if ($i_verifikasi == "false")
				{
				$i_ket = "<font color='white'>Menunggu Verifikasi</font>";
				}
				
			else if ($i_verifikasi == "true")
				{
				$i_ket = "<font color='green'>NASKAH DITERIMA. <br>$i_verifikasi_postdate</font>";
				}
