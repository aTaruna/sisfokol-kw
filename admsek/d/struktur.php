...
$judul = "[PROFIL SEKOLAH] STRUKTUR ORGANISASI";
$judulku = "$judul";
$judulx = $judul;
...
//update cabang
mysqli_query($koneksi, "UPDATE sekolah_struktur_organisasi SET cabang = '$e_cabang' ".
...
if ($_POST['btnSMP'])
	...
	if ((empty($e_kode)) OR (empty($e_jabatan)) OR (empty($e_nama)))
		...
		if ($s == "baru")
			{
			mysqli_query($koneksi, "INSERT INTO sekolah_struktur_organisasi(kd, sekolah_kd, sekolah_kode, sekolah_nama, ".
							"nourut, kode, nama, jabatan, ".
							"tgl_awal, tgl_akhir, postdate) VALUES ".
							"('$x', '$kd81_session', '$e_sekkode','$e_seknama', ".
							"'$e_nourut', '$e_kode', '$e_nama', '$e_jabatan', ".
		...
		if ($s == "edit")
			{
			mysqli_query($koneksi, "UPDATE sekolah_struktur_organisasi SET nourut = '$e_nourut', ".
							"kode = '$e_kode', ".
							"nama = '$e_nama', ".
							"jabatan = '$e_jabatan', ".
							"tgl_awal = '$e_tgl_awal', ".
...
if ($_POST['btnHPS'])
	...
	for ($i=1; $i<=$jml;$i++)
		...
		//del
		mysqli_query($koneksi, "DELETE FROM sekolah_struktur_organisasi ".
						"WHERE sekolah_kd = '$kd81_session' ".
						"AND kd = '$kd'");
		}
...
if (($s == "baru") OR ($s == "edit"))
	...
	$qx = mysqli_query($koneksi, "SELECT * FROM sekolah_struktur_organisasi ".
						"WHERE kd = '$kdx'");
	$rowx = mysqli_fetch_assoc($qx);
	...
	if (empty($kunci))
		{
		$sqlcount = "SELECT * FROM sekolah_struktur_organisasi ".
						"WHERE sekolah_kd = '$kd81_session' ".
