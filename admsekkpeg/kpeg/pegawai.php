...
//update cabang
mysqli_query($koneksi, "UPDATE sekolah_pegawai SET cabang = '$e_cabang' ".
							"WHERE sekolah_kd = '$kd85_session'");
...
mysqli_query($koneksi, "INSERT INTO user(kd, sekolah_kd, sekolah_kode, sekolah_nama, ".
				...
				"('$kd85_session', '$sekkd85_session', '$nip85_session','$xnm85_session', ".
				"'$kd85_session', 'KEPEGAWAIAN', 'KEPEGAWAIAN', ".
				"'SEKOLAH', 'KEPEGAWAIAN', '$today')");
...
if ($_POST['btnIMX'])
	...
	if (empty($filex_namex2))
		...
		if ($ext_filex == ".xls")
			...
			$filex_namex2 = "data_pegawai$sekkd85_session.xls";
			    ...
			    if ($i > 1) 
			    	{
					  ...
					  $i_passx = md5($i_akses);
						...
						mysqli_query($koneksi, "INSERT INTO sekolah_pegawai(kd, sekolah_kd, sekolah_kode, sekolah_nama, ".
										...
										"('$i_xyz', '$sekkd85_session', '$e_sekkode', '$e_seknama', ".
	...
	if ((empty($e_kode)) OR (empty($e_telp)) OR (empty($e_nama)))
		...
		if ($s == "baru")
			...
			mysqli_query($koneksi, "INSERT INTO sekolah_pegawai(kd, sekolah_kd, sekolah_kode, sekolah_nama, ".
							...
							"('$kd', '$sekkd85_session', '$e_sekkode','$e_seknama', ".
...
if ($_POST['btnHPS'])
	...
	for ($i=1; $i<=$jml;$i++)
		...
		//del
		mysqli_query($koneksi, "DELETE FROM user ".
						"WHERE sekolah_kd = '$sekkd85_session' ".
						"AND kd = '$kd'");
		}
