...
if ($_POST['btnSMP'])
	...
	if ((empty($e_kode)) OR (empty($e_ijazah)) OR (empty($e_nama)))
		...
		if ($s == "baru")
			{
			mysqli_query($koneksi, "INSERT INTO majelis_ketua(kd, kode, nama, ijazah, ".
							"tgl_awal, tgl_akhir, postdate) VALUES ".
							"('$x', '$e_kode', '$e_nama', '$e_ijazah', ".
		...
		if ($s == "edit")
			{
			mysqli_query($koneksi, "UPDATE majelis_ketua SET kode = '$e_kode', ".
							"nama = '$e_nama', ".
							"ijazah = '$e_ijazah', ".
							"tgl_awal = '$e_tgl_awal', ".
...
if ($_POST['btnHPS'])
	...
	for ($i=1; $i<=$jml;$i++)
		...
		//del
		mysqli_query($koneksi, "DELETE FROM majelis_ketua ".
						"WHERE kd = '$kd'");
		}
...
if (($s == "baru") OR ($s == "edit"))
	...
	$qx = mysqli_query($koneksi, "SELECT * FROM majelis_ketua ".
						"WHERE kd = '$kdx'");
	...
	if (empty($kunci))
		{
		$sqlcount = "SELECT * FROM majelis_ketua ".
