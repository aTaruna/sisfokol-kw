...
//nilai
$filenya = "uang_keluar.php";
$judul = "[MASTER] Tipe Uang Keluar";
$judulku = "$judul";
$judulx = $judul;
$s = nosql($_REQUEST['s']);
...
if ($_POST['btnSMP'])
	...
	if (empty($e_nama))
		...
		if ($s == "baru")
			{
			//insert
			mysqli_query($koneksi, "INSERT INTO m_uang_keluar(kd, nama, postdate) VALUES ".
							"('$e_kd', '$e_nama', '$today')");
		...
		if ($s == "edit")
			{
			mysqli_query($koneksi, "UPDATE m_uang_keluar SET nama = '$e_nama', ".
							"postdate = '$today' ".
							"WHERE kd = '$e_kd'");
...
if ($_POST['btnHPS'])
	...
	for ($i=1; $i<=$jml;$i++)
		...
		//del
		mysqli_query($koneksi, "DELETE FROM m_uang_keluar ".
						"WHERE kd = '$kd'");
		}
...
if (($s == "baru") OR ($s == "edit"))
	{
	//edit
	$qx = mysqli_query($koneksi, "SELECT * FROM m_uang_keluar ".
									"WHERE kd = '$kd'");
	...
	$sqlcount = "SELECT * FROM m_uang_keluar ".
					"ORDER BY nama ASC";
	$sqlresult = $sqlcount;
